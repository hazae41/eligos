...
const wasm = readFileSync("./wasm/pkg/eligos_bg.wasm")

writeFileSync(`./wasm/pkg/eligos.wasm.js`, `export const data = "data:application/wasm;base64,${wasm.toString("base64")}";`);
writeFileSync(`./wasm/pkg/eligos.wasm.d.ts`, `export const data: string;`);
...
const glueJs = readFileSync(`./wasm/pkg/eligos.js`, "utf8")
  .replace("async function __wbg_init", "export async function __wbg_init")
  .replace("input = new URL('eligos_bg.wasm', import.meta.url);", "throw new Error();")
  .replaceAll("  free() {", disposableJs)
  .replaceAll(beforeMemoryJs, afterMemoryJs)

const glueTs = readFileSync(`./wasm/pkg/eligos.d.ts`, "utf8")
...
writeFileSync(`./wasm/pkg/eligos.js`, glueJs)
writeFileSync(`./wasm/pkg/eligos.d.ts`, glueTs)
